...
| `T` | проверить latency выбранного |
| `D` | удалить (с подтверждением) |
| `Q` / `Esc` | выход |

`●` — активный прокси (зелёный), `▶` — текущий выбор. При активации TUI временно сворачивается, показывает вывод команды, и возвращается по Enter.
